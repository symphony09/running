...
func TestAspectCluster(t *testing.T) {
	...
	if output.Err != nil {
		...
		if !sum.Logs["Aspect1.Base1.Around"][0].End.Before(sum.Logs["Aspect1.Base1.Before"][0].Start) {
			t.Error("Before start before Around end")
		}
		if !sum.Logs["Aspect1.Base1.Before"][0].End.Before(sum.Logs["Aspect1.Base1"][0].Start) {
			t.Error("Base1 start before Before end")
		}
		if !sum.Logs["Aspect1.Base1"][0].End.Before(sum.Logs["Aspect1.Base1.After"][0].Start) {
			t.Error("After start before Base1 end")
		}
		if !sum.Logs["Aspect1.Base1.After"][0].End.Before(sum.Logs["Aspect1.Base1.Around"][1].Start) {
